...
class Display(tk.Tk):
    ...
    def incPrey(self, event) :
        # Quand on réunira les 2 codes, enlever l'ajout automatique, juste lander le code prey.py
        self.prey += 1
        maj = f"Prey : {self.prey}"
        self.labelPrey.config(text = maj)
        exec(open("prey.py").read())
    
    def incPtor(self, event) :
        # Quand on réunira les 2 codes, enlever l'ajout automatique, juste lander le code predator.py
        self.ptor += 1
        maj = f"Predator : {self.ptor}"
        self.labelPtor.config(text = maj)
